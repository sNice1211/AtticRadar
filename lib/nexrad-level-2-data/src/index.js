...
class Level2Radar {
	...
	/**
	 * List all available elevations and their products, scan number, and waveform type
	 */
	...
	/**
	 * Returns an array with the product abbreviations of all of the products contained in the radar file
	 */
	getAllProducts() {
		var lEAP = this.listElevationsAndProducts();

		var allProducts = [];
		for (var i in lEAP) {
			var products = lEAP[i][2];
			for (var n in products) {
				if (!allProducts.includes(products[n])) {
					allProducts.push(products[n]);
				}
			}
		}
